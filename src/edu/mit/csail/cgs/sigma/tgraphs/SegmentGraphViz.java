...
public class SegmentGraphViz extends AbstractRegionPaintable {
	...
	public static void main(String[] args) { 
		...
		Region query = new Region(genome, "11", 522000, 528000);
		...
		try {
			...
			SegmentGraphVizFrame frame = new SegmentGraphVizFrame(graph, query);
			frame.showMe();
	...
	public SegmentGraphViz(SegmentGraph g) { 
		graph = g;
	}
	
	public SegmentGraphViz(SegmentGraph g, Region r) {
		graph = g;
		setRegion(r);
	}
	...
	public void paintItem(Graphics g, int x1, int y1, int x2, int y2) {
		...
		int min = query == null ? -1 : query.getStart(); 
		int max = query == null ? -1 : query.getEnd();
		
		for(String geneID : graph.geneNodes(query)) {  
